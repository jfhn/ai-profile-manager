...
but `--port` and `--base-dir`, so it listens on the target's own default
interface, and the transport's `openEndpoint` is what makes that port reachable
(for a Tailscale transport: HTTPS on the machine's `*.ts.net` name).
...
- **Stop the instance** from the dashboard: apm sends `SIGTERM`, escalates to
  `SIGKILL`, and closes the endpoint, so the URL stops answering.
...
1. On the target: install T3 Code, confirm `t3 --version` works, and make sure
   an apm profile there is active.
...
6. Confirm the URL is **not** reachable from outside the tailnet.
7. **Stop** from the hub. The card returns to `stopped` with no link, and the
   URL stops answering on the second device.
